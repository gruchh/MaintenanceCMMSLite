...
public class SparePartService {
    ...
    public void deleteSparePart(Long id) {
        if (!sparePartRepository.existsById(id)) {
            throw new CmmsLiteApplication.SparePartNotFoundException(String.format(NOT_FOUND, id));
        }
    ...
    private SparePart getSparePartByIdOrThrow(Long id) {
        return sparePartRepository.findById(id)
                .orElseThrow(() -> new CmmsLiteApplication.SparePartNotFoundException(String.format(NOT_FOUND, id)));
    }
}