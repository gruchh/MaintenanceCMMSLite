...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query("SELECT e FROM Employee e JOIN FETCH e.user JOIN FETCH e.employeeRole LEFT JOIN FETCH e.employeeDetails WHERE e.id = :id")
    Optional<Employee> findByIdWithDetails(@Param("id") Long id);

    @Query(value = "SELECT e FROM Employee e JOIN FETCH e.user u JOIN FETCH e.employeeRole er",
